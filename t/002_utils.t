...
try {
    App::Webauth::Role::Utils->spawn_cmd(qw(sleep 1));
}
catch { $error = $_ };
ok( !$error, "external cmd 'sleep 1'" );

undef $error;
try { App::Webauth::Role::Utils->spawn_cmd(qw(sleep 3)) }
catch { $error = $_ };
...
undef $error;
try {
    App::Webauth::Role::Utils->spawn_cmd( qw(ls pipapo),
...
undef $error;
try { App::Webauth::Role::Utils->spawn_cmd(qw(pipapo)) } catch { $error = $_ };
ok( $error, "throws error for unknown command"
);
#diag explain $error;

( $stdout, $stderr ) = App::Webauth::Role::Utils->spawn_cmd(qw(echo asdf));
like( $stdout, qr/^asdf\s*/, 'stdout for external cmd' );

( $stdout, $stderr ) =
  App::Webauth::Role::Utils->spawn_cmd( qw(perl -e), 'warn "foobarbaz\n"' );
...
@ip_addresses =
  map { App::Webauth::Role::Utils->normalize_ip($_) } @ip_addresses;
